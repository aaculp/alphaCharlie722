-- Sample Venue Analytics Data Script (Fixed for Current Schema)
...
-- Add max_capacity if the column exists (from add-max-capacity.sql)
DO $
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'venues' AND column_name = 'max_capacity') THEN
    UPDATE venues SET max_capacity = 85 WHERE id = 'sample-venue-001';
  END IF;
END $;
...
-- Create venue business account (if the table exists)
DO $
BEGIN
...
-- Generate realistic reviews (if reviews table exists)
DO $
DECLARE
  customer_ids TEXT[] := ARRAY[
    'customer-001', 'customer-002', 'customer-003', 'customer-004', 'customer-005',
...
-- Generate favorites (if favorites table exists)
DO $
DECLARE
  customer_ids TEXT[] := ARRAY[
...
-- Generate venue contributions (if venue_contributions table exists)
DO $
DECLARE
  customer_ids TEXT[] := ARRAY[
    'customer-001', 'customer-002', 'customer-003', 'customer-004', 'customer-005',
...
-- Update venue rating based on reviews (if reviews table exists)
DO $
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reviews') THEN
    UPDATE venues 
...
-- Create some recent push notifications for activity feed (if table exists)
DO $
BEGIN
...
BEGIN
  ...
  RAISE NOTICE '   - 15 customer reviews (avg 4.7 stars) - if reviews table exists';
  RAISE NOTICE '   - 20 customer favorites - if favorites table exists';
  RAISE NOTICE '   - User contributions for all venue card types - if table exists';
  RAISE NOTICE '   - 2 push notification campaigns - if table exists';
  ...
  RAISE NOTICE '   Email: owner@analyticsacafe.com';
