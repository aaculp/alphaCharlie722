-- Migration: Create atomic claim function for flash offers
-- Description: Implements atomic claim creation with row locking to prevent race conditions
-- Requirements: Flash Offers MVP - Atomic Claim Logic

-- ============================================================================
-- Atomic Claim Function
-- ============================================================================
-- This function performs the entire claim operation atomically within a transaction
-- It uses SELECT ... FOR UPDATE to lock the offer row and prevent race conditions
...
GRANT EXECUTE ON FUNCTION claim_flash_offer_atomic(UUID, UUID, VARCHAR(6), TIMESTAMPTZ) TO authenticated;

-- Comments
COMMENT ON FUNCTION claim_flash_offer_atomic IS 'Atomically claim a flash offer with row locking to prevent race conditions';

-- Success message
SELECT 'Atomic claim function created successfully!' as message;
